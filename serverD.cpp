...
void respondToQuery(int sockfd, struct sockaddr_in &client_addr, std::unordered_map<std::string, int> &roomAvailability)
{
    ...
    std::cout << "The Server D received an " + queryType + " request from the main server." << std::endl;
    std::string response;
    std::string output;
    std::string conclusion = "The Server D finished sending the response to the main server.";
    ...
    if (queryType == "reservation")
    {
        if (roomAvailability.count(roomCode) > 0)
        {
            if (roomAvailability[roomCode] > 0)
            {
                ...
                conclusion = "The Server D finished sending the response and the updated room status to the main server.";
                response = temp + ",yes";
            }
            else
            {
...
int main()
{
    ...
    std::cout << "The Server D is up and running using UDP on port 42203." << std::endl;
